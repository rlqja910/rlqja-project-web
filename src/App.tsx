    ...
    <div className={`min-h-screen flex bg-transparent text-gray-200 font-pixel selection:bg-purple-500/50 relative ${isMagaMode ? 'border-[8px] border-red-600' : ''}`}>
            ...
            return (
              <div key={main.id} className="space-y-1 mb-2">
                <button 
                  ...
                  className="w-full px-2 py-1.5 flex items-center justify-between text-gray-300 font-pixel mb-1 transition-colors hover:bg-[#2d1b54]/50 rounded"
                >
                        ...
                        <div className={`overflow-hidden transition-all duration-300 ${isSubExpanded ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                          {readySubItems.map(item => (
                            <button
                              ...
                              className={`w-full flex items-center gap-3 px-4 py-2.5 font-pixel text-xs transition-all pl-10 brutal-border-accent brutal-shadow-sm mt-2 ${activeTab === item.id ? 'bg-[#7c3aed]/40 text-cyan-300' : 'bg-[#1a103c] text-gray-400 hover:bg-[#2d1b54]'}`}
                            >
                              <span className="text-sm">{renderIcon(item.icon)}</span> {item.label}
                            </button>
                          ))}
                        </div>
                      </div>
      ...
      <main className="flex-1 flex flex-col min-h-screen min-w-0 overflow-x-hidden">
        ...
        <div className="p-4 sm:p-8 pb-40 md:pb-8 max-w-6xl mx-auto w-full">
          ...
          {activeTab === 'whale' && <WhaleDetectorView />}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-[#1a103c]/95 backdrop-blur-xl border-t-[3px] border-[#7c3aed] z-40 flex justify-around p-2 shadow-[0_-10px_20px_rgba(0,0,0,0.7)]" style={{ paddingBottom: 'max(2rem, env(safe-area-inset-bottom))' }}>
        <button
          onClick={() => handleTabChange('home')}
