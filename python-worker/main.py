...
async def summarize_today_news(timeContext: str = Query(None)):
    try:
        ...
        prompt = (
            ...
            "가독성을 위해 **글머리 기호(-)** 와 **굵은 글씨(**)**를 적극 활용하고, 단락을 명확히 구분해서(줄바꿈 \\n\\n) 눈에 확 들어오게 세련된 마크다운으로 작성해.\n"
            ...
            '  "short_summary": "전체 핵심 상황 1~2줄 요약 (최대 60자 이내로 짧고 강렬하게)",\n'
            '  "detailed_content": [\n'
            '    "### 🔥 종합 브리핑 및 증시 동향\\n\\n이슈 설명 첫번째 문장입니다.\\n\\n두번째 문장입니다.",\n'
...
def process_scout(job_id: str, stockName: str):
    try:
        ...
        return {
            "success": True,
            "combatPower": result_json.get("combatPower", 0),
        ...
        import traceback
        traceback.print_exc()
        jobs[job_id] = {"status": "done", "success": False, "error": str(e)}
        return
...
@app.post("/api/compound-calc")
async def calculate_compound(req: CompoundRequest):
    try:
        lower_stock = req.ticker.strip().lower().replace(" ", "")
        ticker = req.ticker
        if lower_stock in ["스페이스x", "스엑", "spacex"]:
            ticker = "SPCX"
        elif not re.match(r'^[A-Za-z0-9\.\=\^]+$', req.ticker):
            ticker_prompt = (
                f"'{req.ticker}' 기업의 Yahoo Finance 티커만 딱 1개 출력해. 마크다운 금지. JSON 말고 티커명 문자열만 출력해.\n"
                "한국 종목은 .KS 또는 .KQ 붙일것. 예: 삼성전자는 005930.KS, 네이버는 035420.KS"
            )
            ticker_res = model.generate_content(ticker_prompt)
            ticker = ticker_res.text.strip().upper()
            
        stock = yf.Ticker(ticker)
        
        try:
            start_date_obj = datetime.strptime(req.startDate, "%Y-%m-%d")
        except ValueError:
            return {"success": False, "error": "날짜 형식이 잘못되었습니다. YYYY-MM-DD 형식으로 입력하세요."}

        hist = stock.history(start=req.startDate)
        if hist.empty:
            return {"success": False, "error": f"해당 기간의 데이터를 찾을 수 없습니다. (티커: {ticker})"}

        closes = hist['Close'].tolist()
        dates = [d.strftime("%Y-%m-%d") for d in hist.index]

        if not closes:
            return {"success": False, "error": "종가 데이터가 없습니다."}

        initial_price = req.avgPrice if req.avgPrice and req.avgPrice > 0 else float(closes[0])
        final_price = float(closes[-1])

        actual_return_pct = ((final_price / initial_price) - 1.0) * 100.0

        daily_returns = []
        arithmetic_sum_pct = 0.0
        
        if req.avgPrice and req.avgPrice > 0:
            first_day_ret = (float(closes[0]) / req.avgPrice) - 1.0
            daily_returns.append(first_day_ret * 100)
            arithmetic_sum_pct += (first_day_ret * 100)
        else:
        ...
        for i in range(1, len(closes)):
            ...
            arithmetic_sum_pct += (ret * 100)

        compound_effect_pct = actual_return_pct - arithmetic_sum_pct
        
        actual_amount = req.principal * (1 + actual_return_pct / 100.0)
        arithmetic_amount = req.principal * (1 + arithmetic_sum_pct / 100.0)
        compound_diff_amount = actual_amount - arithmetic_amount

        chart_data = []
        curr_arithmetic = 0.0
        for i in range(len(dates)):
            curr_arithmetic += daily_returns[i]
            curr_actual = ((float(closes[i]) / initial_price) - 1.0) * 100.0
            chart_data.append({
                "date": dates[i],
                "actualPct": round(curr_actual, 2),
                "arithmeticPct": round(curr_arithmetic, 2),
        ...
        return {
            "success": True,
            "ticker": ticker,
            "actualReturnPct": round(actual_return_pct, 2),
            "arithmeticReturnPct": round(arithmetic_sum_pct, 2),
            "compoundEffectPct": round(compound_effect_pct, 2),
            "actualAmount": int(actual_amount),
            "arithmeticAmount": int(arithmetic_amount),
            "compoundDiffAmount": int(compound_diff_amount),
