...
class DeployApp(tk.Tk):
    ...
    def git_push(self):
        # 유저 규칙 적용: 항상 pull 받고 커밋&푸쉬 (root 폴더 내 프론트 소스코드 대상)
        cmd = 'git add src public package.json index.html vercel.json tsconfig*.json vite.config.ts && (git commit -m "auto: frontend update" || ver) && git pull origin main --rebase && git push origin main'
    ...
    def backend_deploy(self):
        ...
        cmd = (
            "cd dashboard-app && "
            ".\\gradlew.bat bootJar && "
            "scp -i ..\\LightsailDefaultKey-ap-northeast-2.pem -o StrictHostKeyChecking=no build\\libs\\demo-0.0.1-SNAPSHOT.jar ubuntu@13.124.135.106:/home/ubuntu/demo.jar && "
            "scp -i ..\\LightsailDefaultKey-ap-northeast-2.pem -o StrictHostKeyChecking=no ..\\python-worker\\*.py ubuntu@13.124.135.106:/home/ubuntu/python-worker/ && "
            "ssh -i ..\\LightsailDefaultKey-ap-northeast-2.pem -o StrictHostKeyChecking=no ubuntu@13.124.135.106 \"sudo pkill -9 java || true; sudo fuser -k 8000/tcp || true; pkill -f 'info_board_bot.py' || true; nohup java -Duser.timezone=Asia/Seoul -Djava.security.egd=file:/dev/./urandom -Xmx256m -jar /home/ubuntu/demo.jar </dev/null > /home/ubuntu/java.log 2>&1 & cd /home/ubuntu/python-worker && source venv/bin/activate && nohup uvicorn main:app --host 0.0.0.0 --port 8000 </dev/null > worker.log 2>&1 & nohup python info_board_bot.py </dev/null > info.log 2>&1 &\""
        )
